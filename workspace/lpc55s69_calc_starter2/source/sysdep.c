...
int sys_wait_key(scan_t *scan) {
    ...
    switch (c) {
        ...
        case '\x08':  // Handle backspace
        	case 127:     // Handle delete key (ASCII 127 is DEL)
        		  *scan = backspace;
        		  break;
...
/***** Attend un certain temps en secondes ou jusqu'à une pression de touche.
Retourne le code de scan ou 0 (temps écoulé).
******/
void sys_wait(real time, scan_t *scan)

{
    char c;
    uint32_t start_tick = sys_clock(); // Get the current system clock tick

    // Case 1:
    if (time > 0) {
        while ((sys_clock() - start_tick) < time * 1000) {
...
//*************************	 	carte SD et du système de fichiers FAT 		*************************/
//*************************  												*************************/
//*************************													*************************/
...
char *fs_cd (char *dir)
{
	...
	{
		res = f_chdir(dir); //Change Directory

		   if (res == FR_OK)
		   {
			   res = f_getcwd(cur_path, sizeof(cur_path)); //Get Current Working Directory
	        }
	 }
	return cur_path[0];
}
...
//*************************													*************************/
//*************************  	Lien des fonctions de bas niveau			*************************/
//*************************	 	de la libc avec les foncions		 		*************************/
...
DataBuffer;
volatile DataBuffer * buffer_array;
uint32_t current_buffer_idx;
void init_DataBuffer() {
  buffer_array = (DataBuffer * )(shdata + MAXPOINTS);
}

DataBuffer * allocate_DataBuffer() {
  for (int i = 0; i < BUFFER_CAPACITY; i++) {
      current_buffer_idx = i;
      return & buffer_array[i];
  }
  return NULL;
}
...
const Font * lcd_set_font(DC * dc,
  const Font * font) {

  DataBuffer * DataBuffer = NULL;
  while ((DataBuffer = allocate_DataBuffer()) == NULL) {
    __WFI();
  }
  DataBuffer -> data[0] = (char)((font -> data[0]) & 0xFF);
  DataBuffer -> data[1] = (char)((font -> data[1] >> 8) & 0xFF);
  DataBuffer -> data[2] = (char)(font -> height & 0xFF);
  DataBuffer -> data[3] = (char)((font -> height >> 8) & 0xFF);
  DataBuffer -> data[4] = (char)(font -> width & 0xFF);
  DataBuffer -> data[5] = (char)((font -> width >> 8) & 0xFF);
  while (!mb_push_evt(EVT_SETFONT | (current_buffer_idx << 16), false));
  while (!handshake()) {}
  return NULL;
}
...
uint32_t lcd_set_alignment(DC * dc, uint32_t alignment) {
  ...
  DataBuffer -> data[0] = (char)(alignment & 0xFF);
  DataBuffer -> data[1] = (char)((alignment >> 8) & 0xFF);
  DataBuffer -> data[2] = (char)((alignment >> 16) & 0xFF);
  DataBuffer -> data[3] = (char)((alignment >> 24) & 0xFF);
  while (!mb_push_evt(EVT_SETALIGN | (current_buffer_idx << 16), false));
  while (!handshake()) {}
  return alignment;
}
...
uint32_t lcd_set_direction(DC * dc, uint32_t direction) {
  ...
  DataBuffer -> data[0] = (char)(direction & 0xFF);
  DataBuffer -> data[1] = (char)((direction >> 8) & 0xFF);
  DataBuffer -> data[2] = (char)((direction >> 16) & 0xFF);
  DataBuffer -> data[3] = (char)((direction >> 24) & 0xFF);

  while (!mb_push_evt(EVT_DIRECTION | (current_buffer_idx << 16), false));
  while (!handshake()) {}
  return direction;
}
...
void lcd_draw_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, Color c) {
  ...
  DataBuffer -> data[0] = (char)(x & 0xFF);
  DataBuffer -> data[1] = (char)((x >> 8) & 0xFF);
  DataBuffer -> data[2] = (char)(y & 0xFF);
  DataBuffer -> data[3] = (char)((y >> 8) & 0xFF);
  DataBuffer -> data[4] = (char)(w & 0xFF);
  DataBuffer -> data[5] = (char)((w >> 8) & 0xFF);
  DataBuffer -> data[6] = (char)(h & 0xFF);
  DataBuffer -> data[7] = (char)((h >> 8) & 0xFF);
  DataBuffer -> data[8] = (char)(c & 0xFF);
  DataBuffer -> data[9] = (char)((c >> 8) & 0xFF);
  while (!mb_push_evt(EVT_DRAWRECT | (current_buffer_idx << 16), false));
  while (!handshake()) {}
...
void lcd_draw_segments(SPoint *p , int n, Color c) {
  ...
  if (n < MAXPOINTS) {
    ...
    DataBuffer -> data[0] = (char * ) c;
    DataBuffer -> data[1] = (char * )(c >> 8);
    while (!mb_push_evt(EVT_DRAWSEGMENTS | ((uint16_t) n & 0x7FF) | (current_buffer_idx << 16), false));
    while (!handshake()) {}
  }
}
...
void lcd_draw_path2d(Graph *g, SPoint *p, int n) {
    if (n < MAXPOINTS) {
        while (!mb_push_evt(EVT_DRAWPATH |
                (((uint16_t)g->msize << 20) &
                ((uint16_t)g->mtype << 16) &
                ((uint16_t)g->ltype << 12) &
                (uint16_t)n & 0xFFFFFF),
...
void lcd_fill_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
  ...
  DataBuffer -> data[6] = (char)(h & 0xFF);
  DataBuffer -> data[7] = (char)((h >> 8) & 0xFF);
  DataBuffer -> data[8] = (char)(color & 0xFF);
  DataBuffer->data[9] = (char)((color >> 8) & 0xFF);

  while (!mb_push_evt(EVT_FILLRECT | (current_buffer_idx << 16), false));
  while (!handshake()) {}
...
void lcd_clip(int x1, int y1, int x2, int y2) {
  ...
  DataBuffer -> data[0] = (char)(x1 & 0xFF);
  DataBuffer -> data[1] = (char)((x1 >> 8) & 0xFF);
  DataBuffer -> data[2] = (char)(y1 & 0xFF);
  DataBuffer -> data[3] = (char)((y1 >> 8) & 0xFF);
  DataBuffer -> data[4] = (char)(x2 & 0xFF);
  DataBuffer -> data[5] = (char)((x2 >> 8) & 0xFF);
  DataBuffer -> data[6] = (char)(y2 & 0xFF);
  DataBuffer -> data[7] = (char)((y2 >> 8) & 0xFF);

  // Send event
  while (!mb_push_evt(EVT_CLIP | (current_buffer_idx << 16), false));
  while (!handshake()) {}
}
...
// Function to draw a string on the LCD at specified coordinates (x, y)
void lcd_draw_string(DC *dc, int16_t x, int16_t y, const char *s) {
    DataBuffer *dataBuffer = NULL;

    // Wait until a DataBuffer becomes available
    while ((dataBuffer = allocate_DataBuffer()) == NULL) {
        __WFI(); // Wait for interrupt to save energy
    }

    // x and y coordinates into the DataBuffer
    dataBuffer->data[0] = (char)(x & 0xFF);          // Low x
    dataBuffer->data[1] = (char)((x >> 8) & 0xFF);   // High of x
    dataBuffer->data[2] = (char)(y & 0xFF);          // Low of y
    dataBuffer->data[3] = (char)((y >> 8) & 0xFF);   // High of y

    int i = 4;  // Start at index 4 after the coordinates
    int j = 0;  // Index for the string

    // Copy the string into the DataBuffer
    while (s[j] != '\0') {
        dataBuffer->data[i] = s[j];  // Add each character from the string
        i++;
        j++;

        // handling potential overflow of the DataBuffer
    ...
    // Push the event to the mailbox and wait for handshake
    while (!mb_push_evt(EVT_DRAWSTRING | (current_buffer_idx << 16), false));
    while (!handshake()) {}
}


void lcd_get_string_size(DC * dc,
  const char * s, uint16_t * width, uint16_t * height) {
