...
// Define pointers for FFT input and output
static q15_t *fft_in = NULL;  // Dynamically allocated
static q15_t *fft_out = NULL; // Dynamically allocated
...
header* mpqfft(Calc *cc, header *hd) {
    ...
    fft_out = (q15_t *)malloc(2 * c * sizeof(q15_t));
    ...
    if (hd->type == s_matrix) {
        for (int i = 0; i < c; i++) {
            fft_in[i] = (int32_t)(m[i] * (1 << 9));  // Real values
        }
